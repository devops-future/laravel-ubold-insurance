...
@section('content')
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">@lang('menus.list of sub agents')</li>
                    </ol>
                </div>
                <h4 class="page-title">@lang('menus.list of sub agents')</h4>
            </div>
        </div>
    </div>
    <div class="card">
            ...
            <div class="responsive-table-plugin">
                <div class="table-rep-plugin">
                    <div class="table-responsive">
                        <table id="dataTable" class="table dt-responsive nowrap dataTable no-footer">
                            <thead>
                                <tr>
                                    <th width="20%"> # </th>
    ...
    <div id="add_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="add_form" method="post" action="{{ route('subagents.store') }}">
                    <div class="modal-header">
                        <h4 class="modal-title">@lang('configuration.add type of subagents')</h4>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-info waves-effect waves-light">@lang('global.app_name')</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="edit_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="edit_form" action="/subagents/id" method="post">
                    <div class="modal-header">
                        <h4 class="modal-title">@lang('configuration.edit type of subagents')</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body p-4">
                        {{ csrf_field() }}
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary waves-effect" data-dismiss="modal">@lang('policies.close')</button>
                        <button type="submit" class="btn btn-info waves-effect waves-light" onclick="$('#edit_form').attr('action', '/subagents/'+g_id);">@lang('customers.save change')</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function del(id) {
            swal({
                ...
                if (e.value) {
                    $.ajax({
                        url: "/subagents/destroy/" + id,
                        type: 'post',
                        dataType: 'text',
                        success: function(data) {
                            window.location="{{ route('subagents.index') }}";
                        }
                    });
                }
            });
