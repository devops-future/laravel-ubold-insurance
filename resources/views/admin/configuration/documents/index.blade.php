...
@section('content')
            ...
            <div class="page-title-box">
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        ...
                        <li class="breadcrumb-item active">@lang('menus.type of documents')</li>
                    </ol>
                </div>
                <h4 class="page-title">@lang('menus.type of documents')</h4>
            </div>
        </div>
    </div>
    <div class="card">
    ...
    <div id="add_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="add_form" method="post" action="{{ route('documents.store') }}">
                    <div class="modal-header">
                        <h4 class="modal-title">@lang('configuration.add type of documents')</h4>
    ...
    <div id="edit_modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            <div class="modal-content">
                <form id="edit_form" action="/documents/id" method="post">
                    <div class="modal-header">
                        <h4 class="modal-title">@lang('configuration.edit type of documents')</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body p-4">
                        {{ csrf_field() }}
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary waves-effect" data-dismiss="modal">@lang('policies.close')</button>
                        <button type="submit" class="btn btn-info waves-effect waves-light" onclick="$('#edit_form').attr('action', '/documents/'+g_id);">@lang('customers.save change')</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                if (e.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                            window.location="{{ route('documents.index') }}";
                        }
                    });
                }
            });
